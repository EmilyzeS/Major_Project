...
void SerialInitialise(int baudRate, SerialPort *serial_port) {
 ...
 //set serial port to read and write 
...
void interpretSerial(char * buffer){
  ...
  //parse through the serial information
  for(i = 0; i < BUFFER; i++){
  
    
     //loop through what has been recieved
     if(buffer[i] == 0){
        end = i;
        
        //copy info to a matrix of strings
  ...
  //if a correct header message was scanned it should be formatted like this  
...
//interrupt for message recieved in serial
